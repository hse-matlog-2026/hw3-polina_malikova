...
class Formula:
    ...
    def substitute_operators(self, substitution_map: Mapping[str, Formula]) -> \
        ...
        if is_unary(self.root):
            inside_formula = self.first.substitute_operators(substitution_map)
            if self.root in substitution_map:
                temp = substitution_map[self.root]
                return temp.substitute_variables({'p': inside_formula})
            return Formula(self.root, inside_formula)
